...
| `.github/workflows/deploy-coolify.yml` | GitHub `main` 分支 push 或每日定时调用 Coolify 部署 API，自动触发重新部署 |
...
| Auto deploy | GitHub Actions on `main` push and daily at 04:10 Asia/Shanghai, using `COOLIFY_BASE_URL`, `COOLIFY_TOKEN`, and `COOLIFY_APP_UUID` repository secrets |
...
- GitHub Actions 每天 04:10 Asia/Shanghai 也会触发一次 Coolify 重新部署，用于跟随 `calciumion/new-api:latest` 的官方镜像更新。
